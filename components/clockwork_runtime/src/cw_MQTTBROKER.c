    ...
    switch (event->event_id) {
        case MQTT_EVENT_CONNECTED: {
			...
			list_for_each_safe(&context->subscribers, item, next, list) {
				...
				int msg_id = esp_mqtt_client_subscribe(client, subs->topic, 0);
			}
		}
            break;
...
int cw_MQTTBROKER_handle_message(MachineBase *ramp, MachineBase *machine, int state);
...
void cw_MQTTBROKER_describe(struct cw_MQTTBROKER *m) {
	struct cw_MQTTBROKER_Vars_backup *v = m->backup;
{
	char buf[100];
