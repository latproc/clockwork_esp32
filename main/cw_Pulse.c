...
int cw_Pulse_toggle_speed(struct cw_Pulse *m, ccrContParam) {// toggle_speed 
	m->delay = (1100 - m->delay);
	ESP_LOGI(TAG,"%lld delay: %d", upTime(), m->delay);
	m->machine.execute = 0;
	return 1;
}
...
int cw_Pulse_check_state(struct cw_Pulse *m) {
	//ESP_LOGI(TAG,"%lld pulse checking state; timer: %ld delay: %d", upTime(), m->machine.TIMER, m->delay);
	int new_state = 0; enter_func new_state_enter = 0;
	if (((m->machine.state == state_cw_Pulse_off) && (m->machine.TIMER >= m->delay))) {
		//ESP_LOGI(TAG,"%lld pulse switching on. timer: %ld delay: %d", upTime(), m->machine.TIMER, m->delay);
